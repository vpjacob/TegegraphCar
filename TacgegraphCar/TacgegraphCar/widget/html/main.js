...
apiready = function() {
	...
	FileUtils.readFile("lastweather.json", function(data) {
		if (data.todayTemperature != "") {
			console.log("最后一次天气：" + data.todayTemperature);
			...
			console.log("file---afterTomorrowSky----" + data.afterTomorrowSkycon);
...
//		console.log('console.log($api.jsonToStr(ret))初始化'+$api.jsonToStr(ret));
//		alert('console.log($api.jsonToStr(ret))初始化'+$api.jsonToStr(ret));
		var extra = ret.extra;
		var pushInfo;
		var status;
		if (systemType == 'android') {
			pushInfo = $api.strToJson(extra).pushInfo;
			status = $api.strToJson(pushInfo).status;
		} else if (systemType == 'ios') {
			pushInfo = ret.extra.pushInfo;
			status = ret.extra.pushInfo.status;
		}
		if (status != 3) {//不是天气
	...
	if (systemType == 'android') {
		api.addEventListener({
			...
			console.log('console.log($api.jsonToStr(ret))点击'+$api.jsonToStr(ret));
			if (ret && ret.appParam.ajpush) {
				console.log($api.jsonToStr(ret));
				//alert('291'+$api.jsonToStr(ret));
				var ajpush = ret.appParam.ajpush;
				var id = ajpush.id;
				var title = ajpush.title;
				var content = ajpush.content;
				var extra = ajpush.extra;
				//			console.log(id + "  --title:  " + title + "  content:  " + content + "  extra:  " + extra);
				//alert('测试extras是否能拿到 301'+extra);
				console.log('测试extras是否能拿到 301'+extra);
//				var pushInfo = $api.strToJson(extra).pushInfo;
		...
		api.addEventListener({
			...
			if (ret && ret.appParam.ajpush) {
				var extra = ajpush.extra;
				//			console.log(id + "  --title:  " + title + "  content:  " + content + "  extra:  " + extra);
				var pushInfo = ret.extra.pushInfo;
				//不是天气
				if (ret.extra.pushInfo.status != 3) {
					var relateid = ret.extra.pushInfo.relateId;
					FileUtils.readFile("info.json", function(info, err) {
						var memberid = info.memberid;
						api.openWin({
							name : 'myMessage',
...
function readUserInfoAndKeyInfo() {
	...
	FileUtils.readFile("info.json", function(info, err) {
		...
		userInfo = info;
		//已注册且已登录的用户有钥匙信息,才可以进行开门
		if (userInfo.hasRegist && userInfo.hasLogon) {
			//读取用户的钥匙信息
			FileUtils.readFile("userkeyinfo.json", function(info, err) {
				if (info != null && $api.jsonToStr(info) != '{}') {
					userKeyInfos = info;
				} else {
					userKeyInfos = {};
					$('.key').hide();
...
function setUserKeyInfos() {
	console.log("=============setUserKeyInfos===============");
	//读取info.json文件：
	FileUtils.readFile("info.json", function(info, err) {
		console.log('info.json:  ' + $api.jsonToStr(info));
		if (info.location.address != "") {
			$("#location").html(info.location.address);
		}
		userInfo = info;
...
function loadLocationFormMap(remind) {
	...
	bMap.getLocation({
		...
		console.log('bMap获取地址位置' + $api.jsonToStr(ret));
		if (ret.status) {
			nowlat = ret.lat;
			nowlon = ret.lon;
			...
			geoc.getLocation(point, function(rs) {
				console.log('百度JS定位结果' + $api.jsonToStr(rs));
				var addComp = rs.addressComponents;
				var address = addComp.district + addComp.street + addComp.streetNumber;
				console.log('*********************************   包括' + rs.address + '****' + cityname);
				console.log(rs.address.indexOf(cityname));
				/**
				...
				if (rs.address.indexOf(cityname) >= 0) {
					...
					api.ajax({
						...
						console.log("*******天气********" + $api.jsonToStr(ret));
						if (ret) {
							if (ret.status == "OK") {
...
function getCurrentWeatherAndForecast(lon, lat) {
	...
	console.log("-----天气预报地址----" + forecastUrl);
	$.ajax({
		type : "GET",
		url : forecastUrl,
		success : function(data) {
			if (data.status == "ok") {
				...
				//太阳下山时间
				console.log(daily.astro[0].sunset.time);
	...
	$.ajax({
		...
		url : realtimeUrl,
		success : function(data) {
			console.log("天气::" + $api.jsonToStr(data));
			if (data.status == "ok") {
				...
				console.log("currentSkycon ::: " + currentSkycon);
				//当前温度
