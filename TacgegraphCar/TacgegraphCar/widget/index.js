...
function getUserInfo() {
	...
	FileUtils.readFile("info.json", function(data) {
		if (data != "") {
			...
			console.log(hasRegist + "==22==" + hasLogon + "====" + memberid + "====" + account);
			//是否已登录
			if (hasLogon) {
				console.log("===================21=======================");
				//将用户信息存到全局信息中,待以后使用
				...
				if (connectionType == 'none' || connectionType == "unknown") {
					...
					AjaxUtil.exeScript({
						...
						success : function(data) {
							console.log($api.jsonToStr(data) + "===================43=======================");
...
function openWeatherPage() {
	...
	changeImage("weather");
	console.log("openWeatherPage--------------------start");
	api.openFrame({
		name : 'weather',
		url : 'html/main.html',
		bounces : false,
...
function openCenterPage(show) {
	...
	console.log(hasRegist + "==openCenterPage==" + hasLogon + "====" + memberid + "====" + account);
	var reqUrl = "html/registe/register.html";
	var name = "register";
	//没有登陆的情况下
	if (hasLogon == "false") {
		console.log(hasRegist + "==login==" + hasLogon + "====" + memberid + "====" + account);
		reqUrl = 'html/registe/logo.html';
		name = "login";
		api.openWin({
			name : name,
		...
		//已经登陆的情况下
		console.log(hasRegist + "==room==" + hasLogon + "====" + memberid + "====" + account);
		api.openFrame({
			name : 'room',
